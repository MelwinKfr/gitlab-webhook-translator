...
/** -----
 * Retrieves nested property from object by dot notated string
 * @param {string} propertyName - Dot notated property string
...
function translateUrl(url, gitlabBody) {
    ...
    if (matches !== null) {
        ...
        for (; i < matches.length; i++) {
            var value = 'undefined';
            try {
                value = getProperty(matches[i].substring(1), gitlabBody);
            } catch (e) {
 ...
 * @return {string} - Translation name if defined
 */
function getName(translation) {
    var name = (typeof translation.name === 'undefined') ? 'no_name' : translation.name;
    return (typeof translation.token !== 'undefined' && name === 'no_name') ? translation.token : name;
}
    ...
    process: function (translation, gitlabBody, gitlabToken, done) {
        ...
        try {
            if (typeof translation.condition !== 'undefined') {
                ...
                if (!eval(condition)) {
                    controlPassed = false;
                    log.info('Condition was not met for translation "' + getName(translation) + '"');
                }
            }
        } catch (e) {
